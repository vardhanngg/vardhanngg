[![header](https://raw.githubusercontent.com/vardhanngg/vardhanngg/main/header.png?v=2)](https://www.linkedin.com/in/vardhan-g-882866290/)
# Hi, I'm Vardhan 👋

I'm a Computer Science student interested in understanding how computers work from the ground up.

### 🔧 What I'm Exploring

* 🖥️ Operating Systems & Systems Programming
* ⚙️ Computer Architecture
* 🦀 RISC-V & Bare-Metal Development
* 🔌 Embedded Systems
* 🔐 Cybersecurity
* 💻 C / C++ and low-level programming

### 🚀 Currently Building

* A small user-level operating system.
* RISC-V bare-metal programs and experiments
* Embedded projects with microcontrollers and sensors

### 📚 Always Learning

> Understanding what's happening beneath the abstractions — from software down to hardware.
